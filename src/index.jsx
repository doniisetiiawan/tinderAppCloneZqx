...
export class index extends Component {
  ...
  UNSAFE_componentWillMount = () => {
    this._panResponder = PanResponder.create({
      ...
      onPanResponderRelease: (e, { vx, vy }) => {
        ...
        if (
          Math.abs(this.state.pan.x._value)
          > SWIPE_THRESHOLD
        ) {
  ...
  switchToNextProfile = () => {
    this.resetState();
    const currentPersonId = profiles.indexOf(
      this.state.person,
    );
    const nextPersonId = currentPersonId + 1;
    this.setState({
      person:
        profiles[
          nextPersonId > profiles.length - 1
            ? 0
            : nextPersonId
        ],
    });
  ...
  render() {
    ...
    const nopeTransformStyle = {
      transform: [{ scale: nopeScale }],
      opacity: nopeOpacity,
    };
    const likeTransformStyle = {
      transform: [
        {
          scale: likeScale,
        },
    ...
    return (
      <View style={styles.container}>
        ...
        />
        <Animated.View
          style={[styles.nope, nopeTransformStyle]}
        >
          <Text style={styles.nopeText}>Nope!</Text>
        </Animated.View>
        <Animated.View
          style={[styles.like, likeTransformStyle]}
        >
